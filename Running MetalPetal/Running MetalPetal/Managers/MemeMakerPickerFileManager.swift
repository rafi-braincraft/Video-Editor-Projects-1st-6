...
class MemeMakerPickerFileManager {
   ...
   // private var activeDownloadTasks: [URL: URLSessionDownloadTask] = [:]
    ...
    func getFilePath(folderName: String, fileURL url: String) -> URL {
        ...
        let pickerFolder = documentDirectory.appendingPathComponent("RunningMetal")
        ...
        let fileExtension = url.split(separator: ".").last.map(String.init) // Convert Substring to String
        let fileName = (url.sha256HashValue ?? "default") + "." + (fileExtension ?? "gif") // Handle nil fileExtension
        let filePath = tenorFolder.appendingPathComponent(fileName)
        ...
        if fileManager.fileExists(atPath: filePath.path) {
           ...
           // cancelDownload(url: url)
            ...
            let downloadTask =  URLSession.shared.downloadTask(with: url) { tempLocation, _, error in
                if let error = error {
                    print("Failed to download: \(error.localizedDescription) -->>>",tempLocation)
